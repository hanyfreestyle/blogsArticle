    ...
    <x-admin.card.normal>
      ...
      <form action="{{route($PrefixRoute.'.filter')}}" method="post">
        ...
        <div class="row">
          ...
          <x-admin.form.select-arr name="type" sendvalue="{{old('type',issetArr($getSessionData,'type'))}}"
                                   :send-arr="$ProductType_Arr" label="{{__('admin/proProduct.pro_type')}}" col="3"
                                   :labelview="false"/>

            <x-admin.form.select-arr name="brand_id" sendvalue="{{old('brand_id',issetArr($getSessionData,'brand_id'))}}"
                                     :send-arr="$CashBrandList" label="{{__('admin/proProduct.app_menu_brand')}}" col="3"
                                     :labelview="false"/>

            <x-admin.form.select-arr name="cat_id" sendvalue="{{old('cat_id',issetArr($getSessionData,'cat_id'))}}"
                                     :send-arr="$CashCategoriesList" label="{{__('admin/proProduct.app_menu_category')}}" col="3"
                                     :labelview="false"/>
        </div>
        {{$slot}}
