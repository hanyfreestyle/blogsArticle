...
use App\AppPlugin\Pages\Request\PageRequest;
...
class PageController extends AdminMainController {
    ...
    function __construct(Page $model, PageTranslation $translation,PageCategory $modelCategory, PagePhoto $modelPhoto, PagePhotoTranslation $photoTranslation ) {
        parent::__construct();
...
#|||||||||||||||||||||||||||||||||||||| #     index
    public function index() {
        ...
        if ($this->viewDataTable and $this->yajraTable){
            return view('AppPlugin.Pages.index_DataTable',compact('pageData'));
        }else{
            $rowData = self::getSelectQuery($this->model::def());
            return view('AppPlugin.Pages.index', compact('pageData', 'rowData'));
        }
    }
...
#|||||||||||||||||||||||||||||||||||||| #   DataTable
    public function DataTable(Request $request){
        if ($request->ajax()) {
            $data = $this->model::select(['page_pages.id','photo_thum_1','is_active'])->with('tablename');
...
#|||||||||||||||||||||||||||||||||||||| #     create
    public function create() {
        ...
        $selCat = [];
        return view('AppPlugin.Pages.form',compact('pageData','rowData','Categories','LangAdd','selCat'));
    }
